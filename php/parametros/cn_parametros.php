...
class cn_parametros extends mupum_cn
{
	...
	//---- DT-TIPO-SUBSIDIO -----------------------------------------------------------------------
	//-----------------------------------------------------------------------------------
	function cargar_dt_tipo_subsidio($seleccion)
	{
		if(!$this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->esta_cargada())
		{				// verifica si esta cargada el datos relacion			
			if(!isset($seleccion))
			{
				$this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->cargar();					// lee de la BD fisica y carga al datos relacion
			}else{
				$this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->cargar($seleccion);				// lee de la BD fisica y carga al datos relacion
			}
		}
	}
	function set_cursor_dt_tipo_subsidio($seleccion)
	{
		$id = $this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->get_id_fila_condicion($seleccion);
		$this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->set_cursor($id[0]);
	}

	function hay_cursor_dt_tipo_subsidio()
	{
		return $this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->hay_cursor();
	}

	function resetear_cursor_dt_tipo_subsidio()
	{
		$this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->resetear_cursor();
	}

	function get_dt_tipo_subsidio()
	{
		return $this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->get();
	}

	function set_dt_tipo_subsidio($datos)
	{
		$this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->set($datos);
	}

	function agregar_dt_tipo_subsidio($datos)
	{
		$id = $this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->nueva_fila($datos);
		$this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->set_cursor($id);
	}	

	function eliminar_dt_tipo_subsidio($seleccion)
	{
		$id = $this->dep('dr_parametros')->tabla('dt_tipo_subsidio')->get_id_fila_condicion($seleccion);
